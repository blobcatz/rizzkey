...
describe('API', () => {
	test('success', async () => {
		...
		fetchMock.mockResponse(async (req) => {
			...
			if (req.method == 'POST' && req.url == 'https://rizzkey.test/api/i') {
				if (body.i === 'TOKEN') {
	...
	test('with params', async () => {
		...
		fetchMock.mockResponse(async (req) => {
			const body = await req.json();
			if (req.method == 'POST' && req.url == 'https://rizzkey.test/api/notes/show') {
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: 'https://rizzkey.test/api/notes/show',
			method: 'POST',
	...
	test('204 No Content で null が返る', async () => {
		fetchMock.resetMocks();
		fetchMock.mockResponse(async (req) => {
			if (req.method == 'POST' && req.url == 'https://rizzkey.test/api/reset-password') {
				return { status: 204 };
			} else {
				return { status: 404 };
			}
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: 'https://rizzkey.test/api/reset-password',
			method: 'POST',
