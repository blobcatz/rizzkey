# rizzkey.js
**Strongly-typed official rizzkey SDK for browsers/Node.js.**
...
[![NPM](https://nodei.co/npm/rizzkey-js.png?downloads=true&downloadRank=true&stars=true)](https://www.npmjs.com/package/rizzkey-js)

JavaScript(TypeScript)用の公式rizzkeySDKです。ブラウザ/Node.js上で動作します。
...
便宜上、以後のコード例は上記のように`* as rizzkey`としてインポートしている前提のものになります。

ただし、このインポート方法だとTree-Shakingできなくなるので、コードサイズが重要なユースケースでは以下のような個別インポートをお勧めします。

``` ts
import { api as rizzkeyApi } from 'rizzkey-js';
```
...
## Streaming
rizzkey.jsのストリーミングでは、二つのクラスが提供されます。
