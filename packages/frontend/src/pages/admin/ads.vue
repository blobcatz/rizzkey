...
const ads = ref<rizzkey.entities.Ad[]>([]);
...
rizzkeyApi('admin/ad/list', { publishing: publishing }).then(adsResponse => {
	if (adsResponse != null) {
...
function more() {
	rizzkeyApi('admin/ad/list', { untilId: ads.value.reduce((acc, ad) => ad.id != null ? ad : acc).id, publishing: publishing }).then(adsResponse => {
